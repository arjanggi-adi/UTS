...
import 'package:penjualan_sepeda_poligon/ui/inputpenjualan.dart';
import 'dart:convert';
import 'dart:async';
...
class _HomeState extends State<Home> {
  ...
  Future<List> getData() async {
    final response =
        await http.get("http://192.168.1.9/apiflutter/api/Penjualan");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: new AppBar(
        title: new Text("Penjualan sepeda"),
      ),
      body: createListView(),
      //tombol add
  ...
  Future<http.Response> deletePenjualan(id) async {
    final http.Response response = await http
        .delete('http://192.168.43.44/apiflutter/api/Penjualan/delete/$id');
    return response;
  }
